            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="producto_id" 
                            id="producto_id" 
                            required 
                            disabled {{-- Se deshabilita para evitar cambios de producto en la edición --}}
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed">
                        @foreach ($productos as $producto)
                            <option value="{{ $producto->id }}" {{ old('producto_id', $presentacion->producto_id) == $producto->id ? 'selected' : '' }}>
                                {{ $producto->nombre }}
                            </option>
                        @endforeach
                    </select>
                    <input type="hidden" name="producto_id" value="{{ $presentacion->producto_id }}"> {{-- Campo oculto para enviar el ID --}}
                </div>
