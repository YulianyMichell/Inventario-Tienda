...
@section('title', 'Registrar Nuevo Movimiento de Inventario')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="producto_id" 
                        ...
                        {{-- Ejemplo de productos. En tu controlador, pasa la variable $productos --}}
                        {{-- @foreach ($productos as $producto)
                            <option value="{{ $producto->id }}" {{ old('producto_id') == $producto->id ? 'selected' : '' }}>
                                {{ $producto->nombre }}
                            </option>
                        @endforeach --}}
                        <option value="1">Huevos (Cartón)</option>
                        <option value="2">Café Molido</option>
                    </select>
                    @error('producto_id')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 mt-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">
                        Ej: 70000 COP para 6 cartones de huevos.
                    </p>
                    @error('costo')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
